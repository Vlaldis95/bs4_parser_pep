...
def pep(session):
    ...
    for tr_tag in tr_tags:
        ...
        status = td.text[1:]
        url = find_tag(tr_tag, 'a')
        pep_url = url['href']
        pep_link = urljoin(PEP_URL, pep_url)
        response = get_response(session, pep_link)
        pep_soup = BeautifulSoup(response.text, features='lxml')
        dl = find_tag(pep_soup,
                      'dl',
                      attrs={'class': 'rfc2822 field-list simple'})
        dt = dl.find(string='Status')
        dt_1 = dt.find_parent()
        pep_page_status = dt_1.next_sibling.next_sibling.string
        if pep_page_status not in EXPECTED_STATUS[status]:
            logging.info(
        ...
        statuses.append(pep_page_status)
    count_statuses = Counter(statuses)
    for status, amount in count_statuses.items():
        results.append((status, amount))
    results.append(('Total', count_statuses.total()))
    return results
